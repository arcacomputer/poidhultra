...
Code synchronization and community data synchronization are separate systems. Both sites use the versioned REST API and independent SIWE sessions through their own same-origin proxy. The web adapter is `apps/web/src/utils/communityProxy.ts`; the typed client is `packages/client`. The portable proxy entry point is `@poidh/client/server-proxy`; an optional HTTP gateway supports an original frontend outside Cloudflare. See `docs/integrations/poidh-xyz.md` for the concrete cutover path. The `apps/web/src/trpc/compatibility.ts` bridge covers bounty reads, claims, comments, reactions, albums, profile lookups, and comment moderation; unsupported original procedures must be translated before cutover.
